...
MAX_UPLOAD_BYTES=$(fetch_metadata "max_upload_bytes" || echo "10485760")
if [ "$MAX_UPLOAD_BYTES" -eq 0 ]; then
  NGINX_MAX_BODY_SIZE=0
else
  NGINX_MAX_BODY_SIZE=$((MAX_UPLOAD_BYTES + 5 * 1024 * 1024))
fi
...
sed -i "s/@NGINX_MAX_BODY_SIZE@/${NGINX_MAX_BODY_SIZE}/g" "${INSTALL_DIR}/nginx.conf"
...
DATABASE_POOL_SIZE=$(fetch_metadata "database_pool_size" || echo "20")
DATABASE_MAX_OVERFLOW=$(fetch_metadata "database_max_overflow" || echo "20")
DATABASE_POOL_TIMEOUT=$(fetch_metadata "database_pool_timeout" || echo "30.0")
